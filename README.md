# API Курсов Валют ЦБ (Чистый PHP)

Данный проект предоставляет простой и высокопроизводительный REST API для получения кросс-курсов валют на основе XML-данных Центрального Банка. Проект написан на **чистом PHP 8.2** (без использования тяжелых фреймворков), с применением PostgreSQL, Redis и RabbitMQ для фонового сбора исторических данных.
...
- **Отказоустойчивость и Кеширование**: API сначала пытается прочитать данные из Redis, затем из БД, и только в случае необходимости обращается (с "падением" на прямые HTTP запросы) к API ЦБ напрямую "на лету".
